...
describe('sandboxFlows core loops', () => {
  ...
  it('runBcHealthGatewayConnectorSandbox imports BC sample FHIR', async () => {
    const result = await runBcHealthGatewayConnectorSandbox({
      now: new Date('2026-09-15T12:00:00.000Z'),
    });
    expect(result.importedCount).toBeGreaterThanOrEqual(4);
  ...
  it('runOnPortalConnectorSandbox imports ON sample FHIR', async () => {
    const result = await runOnPortalConnectorSandbox({
      now: new Date('2026-09-15T12:00:00.000Z'),
    });
    expect(result.importedCount).toBeGreaterThanOrEqual(4);
  ...
  it('runMbEchartConnectorSandbox imports MB sample FHIR', async () => {
    const result = await runMbEchartConnectorSandbox({
      now: new Date('2026-09-15T12:00:00.000Z'),
    });
    expect(result.importedCount).toBeGreaterThanOrEqual(4);
  ...
  it('runNsYourHealthConnectorSandbox imports NS sample FHIR', async () => {
    const result = await runNsYourHealthConnectorSandbox({
      now: new Date('2026-09-15T12:00:00.000Z'),
    });
    expect(result.importedCount).toBeGreaterThanOrEqual(4);
