...
const PORTAL_CHROME =
  /\b(portal\s*screenshot|patient\s*portal|mychart|mysask(?:healthrecord)?|my\s*health\s*records|myhealth|ehealth|care\s*connect|health\s*viewer|alberta\s*myhealth|ontario\s*health|telus\s*health|connectingontario|netcare|health\s*gateway|healthgateway|carnet\s*sant[eé]|carnetsante|echart|shared\s*health|mycare\s*noona|yourhealthns|your\s*health\s*ns)\b/i;
